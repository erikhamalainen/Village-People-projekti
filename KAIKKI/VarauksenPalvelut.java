...
public static VarauksenPalvelut haeVarauksenPalvelu (Connection connection, int vid, int pid) throws SQLException, Exception { // tietokantayhteys välitetään parametrina
    ...
    try {
        ...
        lause.setInt( 1, vid);
        lause.setInt(2, pid); // asetetaan where ehtoon (?) arvo
        // suorita sql-lause
    ...
    VarauksenPalvelut palveluOlio = new VarauksenPalvelut ();
...
public int lisaaVarauksenPalvelu (Connection connection) throws SQLException, Exception { // tietokantayhteys välitetään parametrina
    ...
    try {
        ...
        lause.setInt(2, getPalveluId()); // asetetaan where ehtoon (?) arvo, olion varaus_id
        // suorita sql-lause
        tulosjoukko = lause.executeQuery();	
...
public int muutaVarauksenPalvelu (Connection connection) throws SQLException, Exception { // tietokantayhteys välitetään parametrina
    ...
    try {
        ...
        lause = connection.prepareStatement(sql);
        lause.setInt( 1, getVarausId());
        lause.setInt(2, getPalveluId()); // asetetaan where ehtoon (?) arvo
        // suorita sql-lause
        tulosjoukko = lause.executeQuery();	
